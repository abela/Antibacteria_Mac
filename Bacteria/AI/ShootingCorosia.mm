...
#import "Weapon.h"
//
@implementation ShootingCorosia
//
//
-(id) createAtPosition:(CGPoint)position
{
	...
	if((self = [super createAtPosition:position]))
	{
		...
		type = kEnemy;
		enemyAgentType = kShootingCorosia;
		speed = 7.0f;
		fleeSpeed = 5.0f;
		shootRate = ([[GameManager sharedGameManager] bulletinTimeIsOn]) ? 2.0f+(1-[Utils timeScale]) : 2.0f;
		shootDistance = 100;
		fleeDistance = 70.0f;
...
-(void) shooting:(NSTimer*)timer
{
    ...
    if([[GameManager sharedGameManager] gameIsPaused] == YES)
        return;
    //
    Bullet *bullet1 = [weapon shootAtPosition:ccp(sprite.position.x,sprite.position.y)
                                 andRotation:0.0f
    ...
    Bullet *bullet4 = [weapon shootAtPosition:ccp(sprite.position.x,sprite.position.y)
                                ...
                                andStartDelta:100];
    //
    bullet1.tag = SHOOTING_COROSIA_BULLET_TAG;
