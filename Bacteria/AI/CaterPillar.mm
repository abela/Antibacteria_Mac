...
//
#define CATERPILAR_BODIES_COUNT                 20
#define CATERPILLAR_BODY_DELTA                  40.0f
//
@interface CaterPillar(PrivsdateMethods)
@end
//
...
-(id) createAtPosition:(CGPoint)position
{
    if((self = [super init]))
    {
        ...
        enemyAgentType = kCaterpillar;
        speed= 10.0f;
        float deltaX = 0.0f;
        killScore = 300;
        lerpParameter = ([[GameManager sharedGameManager] bulletinTimeIsOn]) ? 0.95f : 0.87f;
        //
        for (int32 i = 0; i < CATERPILAR_BODIES_COUNT; ++i)
        {
            //
            CaterPillarBody *caterpillarBody = [[CaterPillarBody alloc]
                                                    createNextCaterpillarBodyAtPosition:ccp(position.x+deltaX,position.y)];
            //
            caterpillarBody.tag = i;
            caterpillarBody.sprite.opacity = 0;
            //
            bodies.push_back(caterpillarBody);
            deltaX += CATERPILLAR_BODY_DELTA;
            //
            ...
            [caterpillarBody.sprite runAction:[CCSequence actions:fadeIn,fadeInHasDone, nil]];
            //
        }
        //
...
-(void) moveToPosition:(CGPoint) position
{
	...
	bodies[0].sprite.position = ccp(currentPos.x*ASPECT_RATIO,currentPos.y*ASPECT_RATIO);
    //
...
-(void) updateBodies
{
    for(int i = 0;i<(int)bodies.size()-1;i++)
    {
        ...
        else
        {
            ...
            //
            bodies[i+1].sprite.position = newLerpPos;
